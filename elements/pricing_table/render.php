...
$pricing_items     = new sub_form($params->get('pricing_items', ''));
if (!count($pricing_items->get_data())) {
    return false;
}
...
$style->child('.uk-position-cover')->add_css('background-color', $overlay_bg_color['light']);
$style_dark->child('.uk-position-cover')->add_css('background-color', $overlay_bg_color['dark']);
...
$output .= ( $price_description ) ? '<div class="plan-description">' . $price_description . '</div>' : '';
    ...
    foreach ( $pricing_items->get_data() as $key => $item) {
        $text       = $item->params->get('item_title','');
        $text_color   = style::get_color($item->params->get('item_title_color', ''));
        $item_icon_color   = style::get_color($item->params->get('item_icon_color', ''));
        $key++;
        $item_key = '.repeater-item-'.$key.'';
        $style->child(''.$item_key.' .el-content')->add_css('color', $text_color['light']);
        $style_dark->child(''.$item_key.' .el-content')->add_css('color', $text_color['dark']);
        $style->child(''.$item_key.' .pricing-icon')->add_css('color', $item_icon_color['light']);
        $style_dark->child(''.$item_key.' .pricing-icon')->add_css('color', $item_icon_color['dark']);
...
if (!empty($title_border)) {
    style::add_border_style('#'. $element->id . ' .uk-card-title', $title_border, 'global', $element->isRoot);
}
$title_font_style   =   $params->get('title_font_style');
if (!empty($title_font_style)) {
    style::render_typography('#'.$element->id.' .uk-card-title', $title_font_style, null, $element->isRoot);
}
$title_heading_margin   =   $params->get('title_heading_margin', '');
if (!empty($title_heading_margin)) {
    style::set_spacing_style($element->style->child('.uk-card-title'), $title_heading_margin, 'margin');
}
$title_heading_padding   =   $params->get('title_heading_padding', '');
if (!empty($title_heading_padding)) {
    style::set_spacing_style($element->style->child('.uk-card-title'), $title_heading_padding);
}
$title_radius  =   $params->get('title_radius', '');
if (!empty($title_radius)) {
    style::set_spacing_style($element->style->child('.uk-card-title'), $title_radius,'radius');
}

$pricing_font_style   =   $params->get('pricing_font_style');
if (!empty($pricing_font_style)) {
    style::render_typography('#'.$element->id.' .pricing-amount', $pricing_font_style, null, $element->isRoot);
}
$price_margin   =   $params->get('price_margin', '');
if (!empty($price_margin)) {
    style::set_spacing_style($element->style->child('.pricing-value'), $price_margin, 'margin');
}
...
if (json_last_error() === JSON_ERROR_NONE && is_array($icon_size)) {
    $style->child('.ui_icon_on_price')->add_responsive_css('font-size', $icon_size, $icon_size['postfix']);
}
$price_icon_color     = style::get_color($params->get('price_icon_color', ''));
$style->child('.ui_icon_on_price')->add_css('color', $price_icon_color['light']);
$style_dark->child('.ui_icon_on_price')->add_css('color', $price_icon_color['dark']);

$symbol_font_style   =   $params->get('symbol_font_style');
if (!empty($symbol_font_style)) {
    style::render_typography('#'.$element->id.' .pricing-symbol', $symbol_font_style, null, $element->isRoot);
}
$symbol_margin   =   $params->get('symbol_margin', '');
if (!empty($symbol_margin)) {
    style::set_spacing_style($element->style->child('.pricing-symbol'), $symbol_margin, 'margin');
}

$description_font_style   =   $params->get('description_font_style');
if (!empty($description_font_style)) {
    style::render_typography('#'.$element->id.' .plan-description', $description_font_style, null, $element->isRoot);
}

$listing_border    =   json_decode($params->get('listing_border', ''), true);
if (!empty($listing_border)) {
    style::add_border_style('#'. $element->id . ' .pricing-features', $listing_border, 'global', $element->isRoot);
}
$listing_margin   =   $params->get('listing_margin', '');
if (!empty($listing_margin)) {
    style::set_spacing_style($element->style->child('.pricing-features'), $listing_margin, 'margin');
}
$listing_padding   =   $params->get('listing_padding', '');
if (!empty($listing_padding)) {
    style::set_spacing_style($element->style->child('.pricing-features'), $listing_padding);
}

$button_font_style   =   $params->get('button_font_style');
if (!empty($button_font_style)) {
    style::render_typography('#'.$element->id.' .uk-button', $button_font_style, null, $element->isRoot);
}
$button_margin   =   $params->get('button_margin', '');
if (!empty($button_margin)) {
    style::set_spacing_style($element->style->child('.ui-button'), $button_margin, 'margin');
}
$button_padding   =   $params->get('button_padding', '');
if (!empty($button_padding)) {
    style::set_spacing_style($element->style->child('.uk-button'), $button_padding);
}
$button_radius  =   $params->get('button_radius', '');
if (!empty($button_radius)) {
    style::set_spacing_style($element->style->child('.uk-button'), $button_radius,'radius');
}
$button_border    =   json_decode($params->get('button_border', ''), true);
if (!empty($button_border)) {
    style::add_border_style('#'. $element->id . ' .uk-button', $button_border, 'global', $element->isRoot);
}
$button_bg_color     = style::get_color($params->get('button_bg_color', ''));
$style->child('.uk-button')->add_css('background-color', $button_bg_color['light']);
$style_dark->child('.uk-button')->add_css('background-color', $button_bg_color['dark']);

$button_bg_color_hover     = style::get_color($params->get('button_bg_color_hover', ''));
$style->child('.uk-button:hover')->add_css('background-color', $button_bg_color_hover['light']);
$style_dark->child('.uk-button:hover')->add_css('background-color', $button_bg_color_hover['dark']);

$button_color_hover     = style::get_color($params->get('button_color_hover', ''));
$style->child('.uk-button:hover')->add_css('color', $button_color_hover['light']);
$style_dark->child('.uk-button:hover')->add_css('color', $button_color_hover['dark']);
